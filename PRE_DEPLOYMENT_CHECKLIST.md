...
The frontend is publicly deployed and the current 2026 update is locally validated. Complete unchecked items for the exact production candidate.
...
- [x] Missing resume, LinkedIn, and credential links fail safely without placeholder URLs
...
- [x] Contact form uses a transparent `mailto:` fallback; no email-provider secret variables are advertised
- [x] ESLint, TypeScript, production build, 18 backend tests, Chrome E2E, responsive tests, and local API/SEO/CORS smoke tests passed
...
- [x] Configure and verify the public contact email
...
- [x] Obtain the platform-provided frontend HTTPS URL
- [ ] Set `NEXT_PUBLIC_API_URL` to the deployed backend HTTPS origin
- [x] Set `NEXT_PUBLIC_SITE_URL` to the deployed frontend HTTPS origin
- [x] Configure the verified public email; keep LinkedIn hidden until supplied
- [x] Build and deploy the Next.js application from the repository root
...
- [x] Record the production frontend HTTPS URL
